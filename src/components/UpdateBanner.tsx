...
const APP_SLUG = 'nabu-flashcards'
const LS_KEY = 'update-banner-last-seen-nabu-flashcards'
...
function MiniToast({ message, onDone }: { message: string; onDone: () => void }) {
  ...
  return (
    <div
      className={`fixed bottom-20 right-6 z-[110] max-w-xs px-4 py-3 rounded-xl
...
export function UpdateBanner() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          setHasUnread(true)
          setVisible(true)
          setToastMsg(`New update: ${note.title}`)
        },
      )
      .subscribe()
  ...
  return (
    <>
      {/* Self-contained toast */}
      {toastMsg && <MiniToast message={toastMsg} onDone={() => setToastMsg(null)} />}
